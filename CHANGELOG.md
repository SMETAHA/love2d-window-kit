...
- Modal boundary теперь повторно проверяет mouse/touch capture и focus при
  активации нефокусируемого modal-окна, изменении слоя и восстановлении state.
- Заблокированная modal-окном touch-цепочка поглощается от press до release.
- `input.wheel` и `input.touch` отключают источник полностью, а
  `touchscreen.pan` остаётся независимым от pinch и tap-жестов.
- Инерция и smooth touchpad используют FPS-независимую экспоненциальную
  интеграцию без потери `dt` на редких или задержанных кадрах.
- Responsive shrink сохраняет положительную высоту viewport даже для экрана
  ниже title bar.
- Release workflow больше не содержит устаревающее значение версии по умолчанию.
